...
class SellaciousModelProducts extends SellaciousModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		if ($category = $this->app->input->getInt('category_id'))
		{
		...
		$this->state->set('filter.shop_uid', 0);

		if ($shopUid = $this->app->input->getInt('shop_uid'))
		{
			$this->state->set('filter.shop_uid', $shopUid);
		}
		...
		if ($store_id = $this->app->input->getInt('store_id'))
		{
			$this->state->set('filter.seller_uid', $store_id);
		}

		if ($manufacturer = $this->app->input->getInt('manufacturer_id'))
		{
			$this->state->set('filter.manufacturer', $manufacturer);
		}
	...
	{
		$sellerUids = $this->getShippingSellers();

		$dispatcher = $this->helper->core->loadPlugins();
		$dispatcher->trigger('onLoadShippingSellers', array('com_sellacious.products', &$sellerUids));

		if ($sellerUids === false)
		{
			return $this->_db->getQuery(true)->select('a.id')->from($this->_db->qn('#__sellacious_products', 'a'))->where('0');
		}
		...
		{
			$sUids = $this->getSellersByLocation($storeLocation);
			if (is_array($sUids) && count($sUids))
			{
				if (is_array($sellerUids) && count($sellerUids))
				{
					$sellerUids = array_intersect($sellerUids, $sUids);
				}
				else
				{
					$sellerUids = $sUids;
		...
		// Filter by shop
		$shopUid = $this->getState('filter.shop_uid', 0);
		if ($shopUid && !$this->helper->config->get('hide_shop_name_filter'))
		{
			$sellerUids = (array) $shopUid;
		}

		if (is_array($sellerUids) && count($sellerUids))
		{
			$query->where('a.seller_uid IN (' . implode(', ', $sellerUids) . ')');
		}
		elseif ($shopUid || $storeLocation)
		{
			$query->where(0);
		}
		...
		$query->select($variantPrice . ' AS variant_price');
		$query->select($variantPrice . ' + p.product_price AS sales_price');
		...
		if (!$this->helper->config->get('hide_price_filter'))
		{
			...
			{
				$query->where('(' . $variantPrice . ' + p.product_price) * a.forex_rate  >= ' . (int) $price_from);
			}

			// Filter by Max Price
			if ($price_to = $this->getState('filter.price_to'))
			{
				$query->where('(' . $variantPrice . ' + p.product_price) * a.forex_rate <= ' . (int) $price_to);
			}
		}
		...
		$dispatcher->trigger('onAfterBuildQuery', array('com_sellacious.model.products', &$query));
	...
	protected function getItemsQuery()
	{
		...
		if ($language)
		{
			$query->where('(a.language = ' . $db->quote($language) . ' OR a.language = ' . $db->quote('*') . ' OR a.language = ' . $db->quote('') . ')');
		}
		...
		$sub->select('pr.*')
			...
			->where('(pr.client_catid = ' . (int) $catId . ' OR pr.client_catid IS NULL OR pr.client_catid = 0)')
	...
	public function getFilters()
	{
		...
		foreach ($filters as $key => $field)
		{
			...
			$query->clear('where')
				->where('((' . implode(' AND ', $p_where) . ') OR (' . implode(' AND ', $v_where) . '))')
				->where("((table_name = 'products' AND record_id IN (" . $subQueryP . ")) OR (table_name = 'variants' AND record_id IN (" . $subQueryV . ')))');
	...
	/**
	 * Get a list of sellers who are capable of shipping to the selected location code if any
	 *
	 * @return  int[]|bool
	 *
	 * @throws  Exception
	...
	protected function getShippingSellers()
	{
		...
		$sellerUids = $this->_db->setQuery($query)->loadColumn();

		return $sellerUids ?: false;
	}
	...
	/**
	 * Get the filter seller Uids for the store selected location
	 *
	 ...
	 * @since   1.6.0
	 */
	protected function getSellersByLocation($storeLocation)
	{
		$db    = JFactory::getDbo();
		$where = array();
		...
		$filter                = array();
		$filter['list.select'] = 'a.user_id';
		$filter['list.join']   = array(
			array('left', $db->qn('#__sellacious_addresses', 'ad') . ' ON ad.user_id = a.user_id')
		);

		$filter['list.where'][] = 'ad.is_primary = 1';
		...
		$sellerUids = $this->helper->seller->loadColumn($filter);
