		...
		<div class="product-info-box">
			...
			if ($item->price_display == 0):
				...
				if ($price_display > 0 && in_array('products', $price_d_pages)):
					$price = round($item->price_id, 3) > 0 ? $this->helper->currency->display($item->sales_price, $s_currency, $c_currency, true) : JText::_('COM_SELLACIOUS_NOT_AVAILABLE');
			...
			elseif ($item->price_display == 1 && in_array(1, $allowed_price_display)): ?>
				<div class="btn-toggle btn-price-toggle">
					...
					<button type="button" class="btn btn-default hidden" data-toggle="true"><?php
						...
						if ($security):
							$b64text = $this->helper->media->writeText($mobile, 2, true);
			...
			elseif ($item->price_display == 2 && in_array(2, $allowed_price_display)): ?>
				<div class="btn-toggle btn-price-toggle">
					...
					<button type="button" class="btn btn-default hidden" data-toggle="true"><?php
						...
						if ($security):
							$b64text = $this->helper->media->writeText($seller_email, 2, true);
							?><img src="data:image/png;base64,<?php echo $b64text; ?>"/><?php
						else:
							echo $seller_email;
						endif; ?>
