...
JHtml::_('stylesheet', JUri::root() . 'modules/mod_sellacious_latestproducts/assets/css/owl.carousel.min.css');
...
?>
<div class="mod-sellacious-latestproducts latest-carousel-layout <?php echo $class_sfx; ?>">
	<div class="latest-productcarousel owl-carousel" id="latest-productcarousel<?php echo $module->id; ?>">
		<?php foreach ($products AS $product):
			if (($splCategory) && count($latestProductsSpl) > 0):
				if(ModSellaciousLatestProducts::in_array_field($product->id, 'id', $latestProductsSpl)):
				...
				<div class="latest-product-box <?php echo $splCategoryClass; ?>" data-rollover="container">
					...
					<div class="latest-product-info-box">
						...
						if ($seller_attr->price_display == 0)
						{
							...
							{
								$price = round($item->price_id, 3) > 0 ? $helper->currency->display($item->sales_price, $s_currency, $c_currency, true) : JText::_('MOD_SELLACIOUS_LATESTPRODUCTS_NOT_AVAILABLE');
...
<script type="text/javascript">
	jQuery(document).ready(function ($) {
		...
		var owl = $('#latest-productcarousel<?php echo $module->id ?>');
		owl.owlCarousel({
			nav: true,
			navText: [
...
<?php
JHtml::_('script', JUri::root() . 'modules/mod_sellacious_latestproducts/assets/js/owl.carousel.js');
?>
