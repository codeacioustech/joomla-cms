...
JHtml::_('stylesheet', JUri::root() . 'modules/mod_sellacious_sellerproducts/assets/css/owl.carousel.min.css');
...
JHtml::_('stylesheet', 'mod_sellacious_sellerproducts/style.css', null, true);
JHtml::_('stylesheet', 'com_sellacious/fe.view.cart.aio.css', null, true);

?>
<div class="mod-sellacious-sellerproducts sellerproducts-carousel-layout <?php echo $class_sfx; ?>">
	<div class="sellerproducts-productcarousel owl-carousel" id="sellerproducts-productcarousel<?php echo $module->id; ?>">
		<?php foreach ($products AS $product):
		if (($splCategory) && count($sellerProductsSpl) > 0):
			if (ModSellaciousSellerProducts::in_array_field($product->product_id, 'product_id', $sellerProductsSpl)):
			...
			$item                = (object) $item;
			$seller_info         = ModSellaciousSellerProducts::getSellerInfo($product->seller_uid);
			...
			$options = array(
				'title'    => JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_CART_TITLE'),
				'backdrop' => 'static',
			);
			?>

			<div class="item">
				<div class="sellerproducts-product-box <?php echo $splCategoryClass; ?>" data-rollover="container">
					<div class="image-box">
						<a href="<?php echo $url; ?>">
					...
					<div class="sellerproducts-product-info-box">
						<div class="sellerproducts-product-title">
						...
						if ($seller_attr->price_display == 0)
						{
							...
							{
								$price = round($item->price_id, 3) > 0 ? $helper->currency->display($item->sales_price, $s_currency, $c_currency, true) : JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_NOT_AVAILABLE');

								if ($price_display == 2 && round($item->list_price, 3) > 0)
								{
									?>
									<div class="sellerproducts-product-price"><?php echo $price; ?></div>
									<div class="old-price">
										<del><?php echo $helper->currency->display($item->list_price, $s_currency, $c_currency, true); ?></del>
										<span class="sellerproducts-product-offer"><?php echo strtoupper(JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_OFFER')); ?></span>
									</div>
									<?php
								}
								else
								{
									?>
									<div class="sellerproducts-product-price pull-left"><?php echo $price; ?></div>
									<?php
								}
								?>
						...
						elseif ($seller_attr->price_display == 1 && in_array(1, $allowed_price_display))
						{
							...
							<div class="btn-toggle btn-price-toggle">
								<button type="button" class="btn btn-default" data-toggle="true"><?php
									echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_PRICE_DISPLAY_CALL_US'); ?></button>
						...
						elseif ($seller_attr->price_display == 2 && in_array(2, $allowed_price_display))
						{
							...
							<div class="btn-toggle btn-price-toggle">
								<button type="button" class="btn btn-default" data-toggle="true"><?php
									echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_PRICE_DISPLAY_EMAIL_US'); ?></button>
								<button type="button" class="btn btn-default hidden" data-toggle="true"><?php
						...
						{
							$options = array(
								'title'    => (JText::sprintf('MOD_SELLACIOUS_SELLERPRODUCTS_PRICE_DISPLAY_OPEN_QUERY_FORM_FOR',
							...
							<div class="productquerybtn">
								<a href="#query-form-<?php echo $code; ?>" role="button" data-toggle="modal" class="btn btn-primary">
									<i class="fa fa-file-text"></i> <?php echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_PRICE_DISPLAY_OPEN_QUERY_FORM'); ?>
								</a>
							</div>
							<?php
						}
						...
						if (!$features):
							...
							if (count($features)): ?>
								<hr class="isolate">
								<ul class="sellerproducts-product-features">
									<?php
									...
									<?php if ((int) $seller_attr->stock_capacity > 0): ?>
										<?php if ($displayaddtocartbtn == '1'): ?>
											...
											<button type="button" class="btn btn-primary btn-add-cart add" data-item="<?php echo $code; ?>">
												<i class="fa fa-shopping-cart"></i> <?php echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_ADD_TO_CART'); ?>
												(<?php echo (int) $seller_attr->stock_capacity ?>)
											</button>
										<?php endif; ?>
										...
										<?php if ($displaybuynowbtn == '1'): ?>
											<button type="button" class="btn btn-default btn-add-cart buy" data-item="<?php echo $code; ?>" data-checkout="true">
												<i class="fa fa-bolt" aria-hidden="true"></i> <?php echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_BUY_NOW'); ?>
											</button>
										<?php endif; ?>
									<?php else: ?>
										<hr class="isolate">
										<button class="btn lbl-no-stock btn-primary">
											<i class="fa fa-times"></i> <?php echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_OUT_OF_STOCK'); ?>
										</button>
									<?php endif; ?>
								<?php endif; ?>
							</div>
						...
						<?php if ($compare_allow && $displaycomparebtn == '1'): ?>
							<label class="product-compare btn btn-default"><?php echo JText::_('MOD_SELLACIOUS_SELLERPRODUCTS_COMPARE'); ?>
...
<script type="text/javascript">
	jQuery(document).ready(function ($) {
		...
		var owl = $('#sellerproducts-productcarousel<?php echo $module->id ?>');
		owl.owlCarousel({
			nav: true,
			navText: [
...
<?php
JHtml::_('script', JUri::root() . 'modules/mod_sellacious_sellerproducts/assets/js/owl.carousel.js');
?>
