...
class ModSellaciousFiltersHelper
{
	...
	public static function getCategories()
	{
		...
		$filter  = array(
			...
			'list.where'  => array('a.state = 1', 'a.level > 0', $typeFilter),
		);

		$filter['list.start'] = 0;
		$filter['list.limit'] = $helper->config->get('categories_limit', 1);
		...
		return static::buildLevels($categories);
	}
	 ...
	 * @since   1.5.0
	 */
	public static function buildLevels($items)
	{
		$result = array();
	...
	public static function getShopList()
	{
		...
		if ($cat_id)
		{
			...
			$sFilter  = array(
				'list.select' => 'DISTINCT c.product_id',
				'list.join'   => array(
					array('LEFT', '#__sellacious_product_categories c ON c.product_id = a.id'),
				),
				'list.where'  => array(
					'c.category_id IN (' . implode(',', $categories) . ')',
				)
			);
			$pIds = $helper->product->loadColumn($sFilter);
			$sIds = array();
			...
			if (count($pIds))
			{
				foreach ($pIds as $pId)
				{
					$sellerIds = $helper->product->getSellers($pId, 0);

					$sIds = array_merge($sIds, $sellerIds);
				}
			}
			$sIds = ArrayHelper::getColumn($sIds, 'seller_uid');

			if (is_array($sIds) && count($sIds))
			{
				$sIds = array_unique($sIds);
				$filter['list.where'][] = 'a.user_id IN (' . implode(',', $sIds) . ')';
			}
		}
