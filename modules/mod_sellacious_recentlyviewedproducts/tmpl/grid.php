...
?>
<div class="mod-sellacious-recentviewedproducts recent-grid-layout <?php echo $class_sfx; ?>">
	<?php foreach ($products AS $product):
		if (($splCategory) && count($recentProductsSpl) > 0):
			if (ModSellaciousRecentlyViewedProducts::in_array_field($product->product_id, 'product_id', $recentProductsSpl)):
		...
		$sl_params = array(
			'title'    => JText::_('MOD_SELLACIOUS_RECENTLYVIEWEDPRODUCTS_QUICK_VIEW'),
			'url'      => $url_m,
			'height'   => '600',
			'width'    => '800',
			'keyboard' => true,
		...
		<div class="recent-product-wrap-grid">
			<div class="recent-product-box <?php echo $splCategoryClass; ?>" data-rollover="container">
				...
				<div class="recent-product-info-box">
					...
					if ($seller_attr->price_display == 0)
					{
						...
						{
							$price = round($item->price_id, 3) > 0 ? $helper->currency->display($item->sales_price, $s_currency, $c_currency, true) : JText::_('MOD_SELLACIOUS_RECENTLYVIEWEDPRODUCTS_NOT_AVAILABLE');
						...
						<div class="product-action-btn">
							...
							<?php if ($displayquickviewbtn == '1'): ?>
								<a href="#modal-<?php echo $code; ?>" role="button" data-toggle="modal" class="btn btn-default btn-quick-view">
									<i class="fa fa-search"></i> <?php echo JText::_('MOD_SELLACIOUS_RECENTLYVIEWEDPRODUCTS_QUICK_VIEW'); ?>
								</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
