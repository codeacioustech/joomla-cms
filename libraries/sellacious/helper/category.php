...
class SellaciousHelperCategory extends SellaciousHelperBase
{
	 ...
	 * @since   1.5.2
	 */
	public function getCategoryParam($category_id, $key, $default)
	{
		/** @var  Registry[] $cache */
		static $cache = array();
		...
		{
			$params = $this->loadResult(array('id' => $category_id, 'list.select' => 'a.params'));
			$params = new Registry($params);

			$cache[$category_id] = $params;
		}

		return $cache[$category_id]->get($key, $default);
	}
	 ...
	 * @since   1.6.0
	 */
	public function getTranslatedCategory(&$category, $tag = '')
	{
		try
		{
			if (is_object($category))
			{
				...
				$translation    = $this->getCategoryTranslations($category->id);

				if (isset($translation[$tag]))
				{
					if (isset($category->title))
					{
						$category->title = $translation[$tag]['title'];
					}

					if (isset($category->description))
					{
						$category->description = $translation[$tag]['description'];
					}
				}
			}
		}
	...
	/**
	 * Method to get category translations
	 *
	...
	public function getCategoryTranslations($id)
	{
		...
		$refTable     = 'sellacious_categories';

		foreach ($languages as $language)
		{
			$query = $db->getQuery(true);
			$query->select('GROUP_CONCAT(\'{"\', a.reference_field, \'": "\', a.value,\'"}\') as translation');
			$query->from($db->qn('#__sellacious_translations', 'a'));
			$query->where('a.language_code = ' . $db->quote($language->lang_code));
			$query->where('a.reference_id = ' . $id);
			$query->where('a.reference_table = ' . $db->quote($refTable));
			$query->where('a.state = 1');
			$query->group('a.id');

			$db->setQuery($query);

			$translation = $db->loadColumn();
			...
			if (!empty($translation))
			{
				...
				{
					$translationValues = array_merge($translationValues, json_decode($item, true));
				}

				$translations[$language->lang_code] = $translationValues;
			}
		}
