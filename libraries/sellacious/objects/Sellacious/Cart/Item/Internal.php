...
class Internal extends Item
{
	...
	public function check()
	{
		...
		if ($this->cart->getOptions()->get('frontend_stock_check') && $this->getQuantity() > $this->getProperty('stock_capacity'))
		{
		...
		if (($minQ && $this->getQuantity() < $minQ) || ($maxQ && $this->getQuantity() > $maxQ))
		{
			if ($minQ == $maxQ)
			{
				$errors[] = \JText::sprintf('COM_SELLACIOUS_CART_ITEM_ORDER_QUANTITY_MISMATCH', $minQ);
			}
			else
			{
				$errors[] = \JText::sprintf('COM_SELLACIOUS_CART_ITEM_ORDER_QUANTITY_RANGE_MISMATCH', $minQ, $maxQ);
			}
		}
