...
class plgSellaciousRulesProduct extends JPlugin
{
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		if (($name == 'com_sellacious.shoprule' && ArrayHelper::getValue($array, 'sum_method') == 2)
			|| $name == 'com_sellacious.coupon' || $name == 'com_sellacious.shippingrule')
		{
	...
	public function onValidateProductShippingrule($context, Registry $rule, $item, $use_cart = null)
	{
		...
		$product_filter = array_filter((array) $filter->get('products'), 'strlen');
		...
		$mfr_filter = (int) $filter->get('manufacturer');
		...
		// If manufacturer filter is set then determine
		$seller_filter = (int) $filter->get('seller');
		...
		if ($seller_filter)
		{
			...
			elseif ($item->seller_uid != $seller_filter)
			{
				return false;
			}
		}
