...
class plgSellaciousOrder extends JPlugin
{
	...
	public function onAfterOrderPayment($context, $payment)
	{
		...
		{
			$prefix = $payment->state >= 1 ? 'order_payment_success' : 'order_payment_failure';

			$this->createMails($prefix, $payment->order_id);
		}
	}
	...
	protected function parseTemplate($order, &$subject, &$body, $seller = null)
	{
		...
		$replacements = array(
			...
			'payment_method'    => $payment->get('method_name', 'NA'),
			'payment_sandbox'   => $payment->get('test_mode') ? 'TEST MODE' : '',
			'payment_fee'       => $helper->currency->display($payment->get('fee_amount'), $payment->get('currency'), null),
			'payment_amount'    => $helper->currency->display($payment->get('amount_payable'), $payment->get('currency'), null),
			'payment_response'  => $payment->get('response_message'),
