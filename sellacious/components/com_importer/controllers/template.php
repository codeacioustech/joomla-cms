...
class ImporterControllerTemplate extends SellaciousControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($this->helper->access->check('template.edit', $template->import_type, 'com_importer'))
		{
			return true;
		}

		if ($this->helper->access->check('template.edit.own') && $template->created_by == JFactory::getUser()->id)
		{
			return true;
		}
	...
	public function renameAjax()
	{
		...
		try
		{
			...
			if (!$this->helper->access->check('template.edit', $id, 'com_importer') &&
				!($this->helper->access->check('template.edit.own', $id, 'com_importer') && $template->created_by == JFactory::getUser()->id))
			{
	...
	public function removeAjax()
	{
		...
		try
		{
			...
			if (!$this->helper->access->check('template.delete', $id, 'com_importer') &&
				!($this->helper->access->check('template.delete.own', $id, 'com_importer') && $template->created_by == JFactory::getUser()->id))
			{
				throw new Exception(JText::_('COM_IMPORTER_IMPORT_TEMPLATE_ACCESS_DENIED'));
			}
