...
class MenusModelItem extends JModelAdmin
{
	...
	protected function populateState()
	{
		...
		{
			$menuTypeId = 0;
			$clientId   = $app->getUserState('com_menus.items.client_id', 2);
		}
	...
	protected function preprocessForm(JForm $form, $data, $group = 'content')
	{
		...
		$typeFile   = $clientId == 2 ? 'itemadmin_' . $type : 'item_' . $type;
	...
	public function save($data)
	{
		...
		{
			$base   = JUri::base();
			$uri    = JUri::getInstance($base . $data['link']);
	...
	{
		parent::cleanCache('com_menus', 2);
		parent::cleanCache('com_modules');
		parent::cleanCache('mod_menu', 2);
	}
}
