...
class plgSystemSellaciousHyperlocal extends SellaciousPlugin
{
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		if ($form instanceof JForm)
		{
			...
			elseif (($name == 'com_sellacious.user' || $name == 'com_sellacious.profile') && $obj->seller->category_id)
			{
	...
	public function onContentPrepareData($context, $data)
	{
		if ($context == 'com_sellacious.config')
		{
			...
			if (!empty($config) && isset($config[0]))
			{
		...
		elseif ($context == 'com_sellacious.user' || $context == 'com_sellacious.profile')
		{
			...
			{
				$data['seller']['hyperlocal']     = $hyperlocal;
				$data['seller']['timings']        = $timings;
				$data['seller']['delivery_hours'] = $delivery_hours;
				$data['seller']['pickup_hours']   = $pickup_hours;
	...
	public function onContentAfterSave($context, $table, $isNew)
	{
		...
		if ($context == 'com_sellacious.user' || $context == 'com_sellacious.profile')
		{
	...
	public function onAfterBuildQuery($context, &$query)
	{
		...
		elseif ($context == 'com_sellacious.module.recentlyviewedproducts')
		{
			$hyperlocal = $app->getUserState('hyperlocal_location', array());
		...
		elseif ($context == 'com_sellacious.module.sellerproducts')
		{
			$hyperlocal = $app->getUserState('hyperlocal_location', array());
