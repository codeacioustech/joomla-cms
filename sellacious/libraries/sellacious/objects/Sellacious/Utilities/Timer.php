...
class Timer
{
	...
	{
		$now      = microtime(true);
		$dateTime = date('Y-m-d H:i:s T', (int) $now);
		...
		$this->log(\JText::sprintf('COM_SELLACIOUS_TIMER_START', $this->name, $this->start, $dateTime, $note));
	}
	...
	public function hit($count, $interval, $note)
	{
		if ($interval == 0 || ($count > 0 && $count % $interval == 0))
		{
			...
			$this->log(\JText::sprintf('COM_SELLACIOUS_TIMER_HIT', $this->name, $now, $time, $iDuration, $tDuration, $count, $note));
		}
	}
	...
	public function stop($note)
	{
		...
		$this->log(\JText::sprintf('COM_SELLACIOUS_TIMER_END', $this->name, $now, $dateTime, $iDuration, $tDuration, $note));
	...
	public function interrupt($note)
	{
		if ($this->running)
		{
			...
			$this->log(\JText::sprintf('COM_SELLACIOUS_TIMER_INTERRUPT', $this->name, $now, $dateTime, $tDuration, $note));
		}
		else
		{
	...
	{
		file_put_contents($this->log, str_replace("\n", ' ', $entry) . PHP_EOL, FILE_APPEND);
	}
}
