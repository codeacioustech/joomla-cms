...
class SellaciousHelperProduct extends SellaciousHelperBase
{
	...
	public function getSellers($product_id, $only_enabled = true)
	{
		...
		$query->select('a.*')
			...
			->join('inner', '#__users u ON u.id = a.seller_uid')
			->select('s.category_id, s.title AS company, s.code AS seller_code')
			->join('inner', '#__sellacious_sellers s ON u.id = s.user_id')
			->select('sp.currency, sp.mobile')
			->join('inner', '#__sellacious_profiles sp ON sp.user_id = s.user_id');

		// Load desired properties from product table
		$query->select('p.type')
			->join('left', '#__sellacious_products AS p ON p.id = a.product_id')
			->group('a.product_id, a.seller_uid');
	...
	public function getStockHandling($productId = null, $sellerUid = null)
	{
		...
		if (!isset($cache[$keyP]))
		{
			...
			elseif ($handling == 'category')
			{
				try
				{
					...
					list($mode, $stock, $overStock) = $this->helper->category->getStockHandling($categories);
				}
				catch (Exception $e)
				{
				}
			...
			{
				$mode = $handling == 'product' ? true : false;
			}
			...
			$cache[$keyP] = array($mode, $stock, $overStock);
		}
		...
		if (!isset($cache[$keyS]))
		{
			list($mode, $stock, $overStock) = $cache[$keyP];
			...
			$cache[$keyS] = array($mode, $stock, $overStock);
		}
